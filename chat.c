...
static void usage() {
	fputs("chat - irc it - " VERSION "\n"
	      "usage: chat [-i <irc dir>] [-s <host>] [-p <port>]\n"
...
static void create_filepath(char *filepath, size_t len, char *channel, char *suffix) {
	if(!get_filepath(filepath, len, striplower(channel), suffix)) {
		fputs("chat: path to irc directory too long\n", stderr);
		exit(EXIT_FAILURE);
	}
}
...
static void add_channel(char *cname) {
	...
	if(fd == -1) {
		printf("chat: exiting, cannot create in channel: %s\n", name);
		exit(EXIT_FAILURE);
	}
	c = calloc(1, sizeof(Channel));
	if(!c) {
		perror("chat: cannot allocate memory");
		exit(EXIT_FAILURE);
	}
...
static int tcpopen(unsigned short port) {
	...
	if(!hp) {
		perror("chat: cannot retrieve host information");
		exit(EXIT_FAILURE);
	}
	...
	if((fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		perror("chat: cannot create socket");
		exit(EXIT_FAILURE);
	}
	if(connect(fd, (const struct sockaddr *) &sin, sizeof(sin)) < 0) {
		perror("chat: cannot connect to host");
		exit(EXIT_FAILURE);
	}
	return fd;
}
		...
		case 'l':
			...
			if(buf[2] == ' ' && strlen(buf)>=3)
				...
				snprintf(message, PIPE_BUF,
						"PART %s :chat - 500 SLOC are too much\r\n", c->name);
			write(irc, message, strlen(message));
			close(c->fd);
...
static void handle_server_output() {
	...
	if(read_line(irc, PIPE_BUF, buf) == -1) {
		perror("chat: remote host closed connection");
		exit(EXIT_FAILURE);
	}
	proc_server_cmd(buf);
}
...
static void run() {
	...
	for(;;) {
		...
		if(r < 0) {
			if(errno == EINTR)
				continue;
			perror("chat: error on select()");
			exit(EXIT_FAILURE);
		} else if(r == 0) {
			if(time(NULL) - last_response >= PING_TIMEOUT) {
				print_out(NULL, "-!- chat shutting down: ping timeout");
				exit(EXIT_FAILURE);
			}
